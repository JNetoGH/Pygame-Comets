...
class TextHolder(GameObject):
    def __init__(self, scene, rendering_layer):
        ...
        for i in range(0, self.total_scores_for_exhibition):
            name = TextRenderComponent(self.scores[i][0], self.font_size, self.color, -100, self.initial_pos_y + self.spacing_y * i, self)
            score = TextRenderComponent(self.scores[i][1], self.font_size, self.color, 100, self.initial_pos_y + self.spacing_y * i, self)
